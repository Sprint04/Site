...
insert into
	funcao value (null, 'Teste', 'Fazer testes');
...
insert into
	projeto value (
		...
		'Desenvolver uma api para monitorar hardwares de computadores'
	);
...
values
	...
	(
		null,
		'Aprovação',
		'Esperando por aprovação de alguem'
	),
...
insert into
	atribuicao(fkProjeto, fkGrupo, fkSituacoes) value (1, 1, 6);
...
insert into
	complemento value (null, 1, 2435, 'Predio bem grande');
...
insert into
	tokens value (null, '123451234512345', 1);
...
insert into
	cargo value (
		null,
		'Gerente de QA',
		'Gerenciar tudo sobre o projeto',
		1
	);
...
insert into
	permissao value (null, 'Criar', 'Oque ela faz'),
...
insert into
	permissionamento value (null, 1, 1, true),
	(null, 1, 2, true),
	(null, 1, 3, true),
	(null, 1, 4, true);
...
insert into
	tipoTelefone value (null, 'Celular', 9),
	(null, 'Fixo', 8),
	(null, 'Ramal', 3);
...
values
	(
		null,
		'Porcentagem',
		'Usado como medida de volume indica que o total foi dividido por 100 e aquilo e tantas partes daquela divisão'
	),
	(null, 'Unidade', 'Usado como medida unitaria'),
	(
		null,
		'hertz',
		'Usado como medida na informatica de velocidade mas é uma medida de frequencia'
	),
	(
		null,
		'Gigahertz',
		'Usado como medida na informatica de velocidade mas é uma medida de frequencia, é equivalente a 10^9 hertz'
	),
	(
		null,
		'bytes',
		'Usado como medida de transferencia de dados'
	),
	(
		null,
		'Gigabytes',
		'Usado como medida de transferencia de dados, equivalente a 10^9 bytes'
	),
	(
		null,
		'Megabytes',
		'Usado como medida de transferencia de dados, equivalente a 10^6 bytes'
	);
...
values
	(
		null,
		'Utilizada para ver todos que falam quanto esta sendo usado disso',
		1
	),
	...
	(
		null,
		'Utilizada para ver quantidade conectada',
		11
	);
...
values
	(
		null,
		'Alerta Vermelho',
		'Deu ruim cara, tem que ver urgente isso ai'
	),
...
CREATE TABLE Monitorar (
	...
	permissao BOOLEAN,
	FOREIGN KEY (fkPlano) REFERENCES plano(idPlano),
	FOREIGN KEY (fkHardware) REFERENCES tipoComponente(idTipoComponente),
...
insert into
	componentes value (null, 'CPU 1', 'Um componente do escopo', 1),
...
create table estadoComponente(
	idEstadoComponente int primary key auto_increment,
	estado varchar(45),
	descricao varchar(200)
);
...
insert into
	estadoComponente
values
	(null, 'Instalado', 'Esta instalado e ativado'),
	(
		null,
		'Manutenção',
		'Esta desinstalado e arrumando'
	),
	(
		null,
		'Desinstalado',
		'Não esta instalado e guardado'
	),
	(
		null,
		'Sem Funcionar',
		'Instalado e não funcionando'
	),
	(
		null,
		'Aguardando Manutenção',
		'Desinstalado e aguardando ser arrumado'
	);

create table componenteConectado(
	idComponenteConectado int auto_increment,
	fkDispositivo int,
	foreign key (fkDispositivo) references dispositivo(idDispositivo),
	fkComponente int,
	foreign key (fkComponente) references componentes(idComponente),
	fkEstadoComponente int,
	foreign key (fkEstadoComponente) references estadoComponente(idEstadoComponente),
	primary key(idComponenteConectado, fkDispositivo, fkComponente)
);

-- Este select irá me ajudar a me retornar o id do endereço
...
complemento.numero AS 'Numero',
complemento.complemento AS 'Complemento'from usuario join Empresa on usuario.fkEmpresa = empresa.idEmpresa
	JOIN complemento ON  empresa.fkEnderecoComplemento = complemento.idComplemento
		 JOIN endereco ON complemento.fkEndereco = endereco.idEndereco WHERE usuario.idUsuario = 6;
         
select * from empresa WHERE cnpj = "34924003000133";
SELECT * FROM usuario WHERE cpf = "54415406807";
         
select * from complemento WHERE numero = '116' AND complemento = 'A';
select * from complemento JOIN endereco on fkEndereco = idEndereco WHERE idComplemento = 2;
DELETE FROM endereco WHERE idEndereco = 31;
